        ...
        <div class="d-flex justify-content-between align-items-center">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item fs-5 fw-500">
                    <a href="{{ route('admin.job_opening.index') }}">Job Opening</a>
                </li>
            </ol>
                    ...
                    <div class="card-body box-shadow">
                        <div class="table-responsive" id="table-display">
                            <table class="table table-striped table-bordered zero-configuration">
                                ...
                                <tbody>
                                    @foreach ($job_openingsdata as $key => $value)
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            <td>{{ $value->title }}</td>
                                            ...
                                            <td>
                                                @if ($value->is_available == 1)
                                                    <button class="btn btn-sm btn-success rounded-4" tooltip="Active"
                                                        onclick="StatusUpdate('{{ $value->id }}','2','{{ route('admin.job_opening.status') }}')">
                                                        <i class="fa-sharp fa-solid fa-check"></i>
                                                    </button>
                                                @else
                                                    <button class="btn btn-sm btn-danger rounded-4" tooltip="Inactive"
                                                        onclick="StatusUpdate('{{ $value->id }}','1','{{ route('admin.job_opening.status') }}')">
                                                        <i class="fa-sharp fa-solid fa-xmark"></i>
                                                    </button>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.job_opening.edit', $value->id) }}" tooltip="Edit"
                                                ...
                                                <button class="btn btn-sm btn-danger rounded-4" tooltip="Delete"
                                                    onclick="Delete('{{ $value->id }}','{{ route('admin.job_opening.delete') }}')">
